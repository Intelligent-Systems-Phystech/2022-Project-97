...
$$\mathfrac{D}_2 =  \{(\mathbf{x}'_i, y'_i)\}_{i=1}^{m_2},~\mathbf{x}'_i \in \mathbb{R}^{n_2},~y'_i \in \mathfrac{C}_2 = \{1, \dots, c_2\}.$$

Set $\mathfrac{D}_2$ is \textit{more complex} than $\mathfrac{D}_1$, i.e., $n_2 > n_1$ or $c_2 > c_1$.
...
$$\hat{\mathbf{u}} =  \underset{\mathbf{u}}{\arg\min}~\mathcal{L}_g(\mathbf{u}, \mathfrac{D}_1) =\underset{\mathbf{u}}{\arg\min}~\sum\limits_{i=1}^{m_1} l \bigl(y_i,~g(\mathbf{x}_i, \mathbf{u})\bigr),$$
